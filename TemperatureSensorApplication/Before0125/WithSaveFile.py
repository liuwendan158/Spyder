# -*- coding: utf-8 -*-
"""
Created on Tue Nov 21 11:11:00 2017
...
def SaveFile(list):
    ...
    print(excelrow)
    #workbook.close()
    #np.savetxt(r"C:\Users\user\Desktop\SensorData.csv", list, delimiter=",", fmt='%.4f')
    ...
    while 1:
        for i in range(50):
            ...
            if(data1==b'\xff\xff'):
                ...
                ser.read(1)
                data1=0
                ListData=np.reshape(TemData,2500)
                SaveFile(ListData)
        yield TemData
...
for i in range(20):
    ...
    plt.clf()
    M=next(rw)
    CS=plt.contourf(X, Y, M, 100,origin='upper', cmap=plt.cm.jet,vmin=25, vmax=35)
    ...
    for (j,h),label in np.ndenumerate(M):
        plt.text(h,j,float("%.1f" % label),ha='right',va='bottom',fontsize=8,color='white')
    rightnow=time()
